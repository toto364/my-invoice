...
export function ResourceRenderer(props: ResourceRendererProps) {
  ...
  return (<>
    ...
    {hasTableView ? <table className={clsx(
        ...
        <thead className="rounded-lg text-left text-sm font-normal">
          <tr>
            {props.headers.map((header, idx) => (
              <th key={`${idx}`} scope="col" className={clsx(
                idx > 1 ? "px-3 py-5 font-medium" : "px-4 py-5 font-medium sm:pl-6",
                header.className,
              )}>
                {header.children}
              </th>
